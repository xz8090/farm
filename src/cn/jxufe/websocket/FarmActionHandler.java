...
public class FarmActionHandler extends TextWebSocketHandler{  
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {  
        users.add(session);  
        User farmUser = (User) session.getHandshakeAttributes().get("curUser");
        LOGGER.info("用户 " + farmUser.getUsername() + " 成功建立连接");
    }  
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {  
    	User farmUser = (User) session.getHandshakeAttributes().get("curUser");        
        LOGGER.info("用户 " + farmUser.getUsername() + " 连接关闭。状态: " + status);  
    ...
    public void handleTransportError(WebSocketSession session, Throwable exception) throws Exception {
        ...
        LOGGER.debug("用户: " + farmUser.getUsername()  + " 连接由于传输错误被关闭......");  
        users.remove(session);  
    }
